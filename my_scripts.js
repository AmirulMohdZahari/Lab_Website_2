...
///*
					...
					for(row_counter = 0; row_counter < table.rows.length; row_counter++)
					{
						for(col_counter = 0; col_counter < table.rows[row_counter].cells.length; col_counter++)
						{
							...
							if (cell_value == parseInt(cell_value)) {
								cell_name = table.rows[row_counter].cells[col_counter-1].innerHTML
								col_counter++;
								...
								if (cell_value > cell_value2) {
									...
									table.rows[row_counter].cells[col_counter].innerHTML = cell_name;
								}
								total_counter++;
							}
						}
					...
					document.getElementById("losses").innerHTML = total_counter - win_counter;
				}
//*/
...
///*
					function loadPlayersPage() {
						...
						for(var i = 0; i<players.length;i++) {
							var anchor = "<a name='option' class = 'dropdown-item' href='#' onclick = 'switchPlayers("+i+")'>"+//+ plyr +"</a>"
							players[i].name +"</a>"
							insert = insert + anchor;
						}
...
//*/
					function switchPlayers(playerNum) {
   					...
   					document.getElementById("rec_yards").innerHTML= players[playerNum].receiving_yards;
						var avg_pass = Math.round(players[playerNum].pass_yards/players[playerNum].games_played);
   					var avg_rush = Math.round(players[playerNum].rushing_yards/players[playerNum].games_played);
   					var avg_rec = Math.round(players[playerNum].receiving_yards/players[playerNum].games_played);
